...
export const useFileSync = (bookKey: string) => {
  ...
  const pushNow = useCallback(async () => {
    ...
    // FORK: transient books (feed articles, born tombstoned) must never create
    // Readest/books/<hash>/ residue on the remote — the library drift-scan later
    // re-adopts that residue as phantom shelf entries.
  ...
  const pullNow = useCallback(async (): Promise<boolean> => {
    ...
    if (remoteProgressApplied(config.location, working.location)) {
      ...
      // FORK: also MOVE the open reader to the pulled-ahead position — upstream's
      // remoteProgressApplied only surfaces the hint, it never navigates, so
      // switching devices looked like sync did nothing until the next reopen.
      // Forward-only (never yank backward) and never during a deep-link preview.
      const localCFI = config.location;
      const remoteCFI = working.location;
      if (localCFI && remoteCFI && !isMalformedLocationCfi(remoteCFI)) {
        ...
        const navView = getView(bookKey);
        const isPreview = useReaderStore.getState().getViewState(bookKey)?.previewMode;
        if (remoteAhead && navView && !isPreview) navView.goTo(remoteCFI);
      }
    }
