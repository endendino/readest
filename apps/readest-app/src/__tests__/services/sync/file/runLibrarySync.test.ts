...
describe('getReadyFileSyncBackends', () => {
  ...
  // FORK: the premium paywall is pinned OFF (utils/access.ts) — a no-login
  // self-hosted build is always the 'free' plan, so the gate must NOT pause
  // third-party sync. Upstream asserts [] here.
