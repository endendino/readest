 ...
 *
 * This is "reverse-proxy injects auth" mode: the app points at a same-origin
 * path (e.g. https://host/dav) that a gateway (Caddy basic_auth) protects and
 * whose upstream Authorization it rewrites server-side. With no Authorization
 * set on the fetch, the browser's cached app-login credential flows through for
 * the gateway's check; sending an empty header would clobber it and 401. When
 * credentials ARE present (direct WebDAV, e.g. the native app) we send them as
 * before.
 */
