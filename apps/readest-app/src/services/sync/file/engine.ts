...
export class FileSyncEngine {
  ...
  async syncLibrary(books: Book[], options: SyncLibraryOptions): Promise<SyncLibraryResult> {
    ...
    // FORK note: the fork's hand-rolled remote-tombstone propagation was
    // removed here — upstream 0.11.18 ships its own deletion propagation
    // (#4856/#4860, the local-delete pass above), which covers it.
    ...
    if (canPull && (!remoteIndexUnchanged || fullSync)) {
      ...
      try {
        ...
        for (const entry of dirEntries) {
          ...
          // FORK: never re-adopt a dir whose index row is tombstoned — that is
          // residue of a deleted book, not drift (the phantom-shelf incident).
          if (!allBooksMap.has(entry.name) && !remoteDeleted.has(entry.name)) {
