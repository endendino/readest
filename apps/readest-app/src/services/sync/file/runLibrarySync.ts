...
const syncOneBackend = async (
  ...
  // FORK: transient books (feed articles) live only in the in-memory library
  // for the open reader — they must never reach the shared cloud index (the
  // 190-article shelf-pollution incident). Filter them out of every push pass.
  const libraryBooks = useLibraryStore.getState().library.filter((b) => !b.transient);
  const result = await engine.syncLibrary(libraryBooks, {
